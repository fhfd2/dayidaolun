...
const devicesId = "646304385" // 填写在OneNet上获得的devicesId 形式就是一串数字 例子:9939133
const api_key = "yZ6=6Kzsi168gc3hgrB90StbbYQ=" // 填写在OneNet上的 api-key 例子: VeFI0HZ44Qn5dZO14AuLbWSlSlI=
...
Page({
  ...
  get11:function(){
    var that=this
    wx.request({
      url: `https://api.heclouds.com/devices/646304385/datapoints?datastream_id=Temperature,Humidity`,
      method: 'GET',
      header: {
        'api-key': "yZ6=6Kzsi168gc3hgrB90StbbYQ="//写入你OneNet上的Apikey
      },
      data:"dhtguk:6",
      success: (res) => {
         console.log(res.data)
         that.setData({temp:res.data.data.datastreams[0].datapoints[0].value})//0代表温度数据
         that.setData({humid:res.data.data.datastreams[1].datapoints[0].value})//1代表湿度数据
        }
    })
  },
